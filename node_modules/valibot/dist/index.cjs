...
__export(src_exports, {
  ...
  getErrorAndPipe: () => getErrorAndPipe,
  includes: () => includes,
  instance: () => instance,
...
function strict(schema, error) {
  return {
    ...
    parse(input, info) {
      ...
      if (Object.keys(input).length !== Object.keys(output).length) {
        throw new ValiError([
          {
            reason: "object",
            validation: "strict",
...
function strictAsync(schema, error) {
  return {
     ...
     */
    async parse(input, info) {
      const output = await schema.parse(input);
      if (Object.keys(input).length !== Object.keys(output).length) {
        throw new ValiError([
          {
            reason: "object",
...
function toMaxValue(requirement) {
  return (input) => input > requirement ? requirement : input;
}
